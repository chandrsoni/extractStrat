...
x[:, 2] = np.arange(size)
x = x.T
y = np.random.uniform(lowvalue, highvalue, (size, 1))

n2Count = 250
w3 = np.random.randn(n2Count, nparam)
...
def forward(x, w1, b1, w2, b2, w3, b3, size):
    # forward function
...
output = forward(x, w1, b1, w2, b2, w3, b3, size)
